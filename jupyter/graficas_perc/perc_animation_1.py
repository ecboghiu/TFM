...
def r_psi(theta,dt):
    z = np.sum(np.exp(theta*1j),axis=0)/len(theta)
    lenz=len(z)
    ...
    weff_med = np.sum(weff_med,axis=0)/len(theta)
...
random.seed(22)
np.random.seed(77)
w=2.5+(0.5--0.5)*np.random.rand(NODE_NR)
#w=np.random.normal(3, 0.5, NODE_NR)
...
edge_number = 300 #int(0.5*total_edge_number)
l = []
c = []
...
# for achlioptas
'''
nx.draw_networkx_edges(Gcc[2], pos=POS_FINAL,
       with_labels=False,
       edge_color='blue',
       width=7.0,
       alpha=0.6
...
nx.draw_networkx_edges(Gcc[5], pos=POS_FINAL,
       with_labels=False,
       edge_color='blue',
       width=7.0,
       alpha=0.6
...
nx.draw_networkx_edges(Gcc[11], pos=POS_FINAL,
       with_labels=False,
       edge_color='blue',
       width=7.0,
       alpha=0.6
...
nx.draw_networkx_edges(Gcc[4], pos=POS_FINAL,
       with_labels=False,
       edge_color='blue',
       width=7.0,
       alpha=0.6
...
ccmap_aux
#cyan_purple_cmap
#sm = plt.cm.ScalarMappable(cmap=psi_purple_cmap, norm=plt.Normalize(vmin=-np.pi, vmax=np.pi))
...
plt.savefig('perc'+str(edge_number)+'_withW.png',dpi=1200)
...
omega=w
colors_w = frequency_to_color(omega)
...
def update_argand(i):
    ...
    for node in range(0,NODE_NR):
        plt.scatter(np.cos(asd[node][i]),np.sin(asd[node][i]),facecolor=colors_w[node], # color='b',
                marker='o', linewidth=0.5, zorder=3, edgecolor='gray', s = 150)
    ...
    '''
    sm = plt.cm.ScalarMappable(cmap=cyan_purple_cmap, norm=plt.Normalize(vmin=min(omega), vmax=max(omega)))
    sm._A = []
    cb1 = plt.colorbar(sm)
...
nx.draw(g, pos=POS_FINAL, node_size=150, node_color = colors_s,
        edge_color = 'gray',
        with_labels=False, font_size=10, font_color='white')
...
frame = [i for i in range(0,int(len(t)))]
...
#ani = animation.FuncAnimation(fig, update_argand, frames=frame, blit=True)
ani = animation.FuncAnimation(fig, update_graph, frames=frame, blit=False)

#ani.save('cool_animation17_dpi-1_bitrate-1_w3_0.18_fireflies_0.005.mp4', bitrate = -1, fps=100, extra_args=['-vcodec', 'libx264'])
