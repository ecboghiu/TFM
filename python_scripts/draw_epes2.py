...
def plot_r_thing(data_labels_colors):
    ...
    j=len(plot_data)
    plt.figure(figsize=(2*one_column_figure_size * golden_ration, 2*one_column_figure_size))
    ...
    #plt.gcf().subplots_adjust(bottom=0.15)
    plt.savefig("graph_aux_r_"+str(j)+".png",dpi=1200)
...
def plot_phase_diag(data_labels_colors):
    ...
    for i in range(0,len(plot_data)):
        ...
        ax.scatter(plot_data[i][2*temp_len-t1:t2,0],
            ...
            s = 35, #label = labels[i][0],
            c = 'cyan',# plot_data[i][2*temp_len-t1:t2,2],
